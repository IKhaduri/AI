...
def enhancedFeatureExtractorDigit(datum):
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if matrix[(x, y)] == 0:
                cur_color += 1
                cnt_colors(datum, x, y, cur_color, matrix)
...
def cnt_colors(datum, x, y, color, matrix):

    if x >= DIGIT_DATUM_WIDTH or y >= DIGIT_DATUM_HEIGHT:
        return
    if matrix[(x, y)] == color:
        return
    matrix[(x, y)] = color
    for dx, dy in adjacency:
        if -1 < x+dx < DIGIT_DATUM_WIDTH and -1 < y+dy < DIGIT_DATUM_HEIGHT:
            if datum.getPixel(x, y) > 0 and datum.getPixel(x+dx, y+dy) > 0 or \
                                    datum.getPixel(x, y) == 0 and datum.getPixel(x + dx, y + dy) == 0:
                cnt_colors(datum, x+dx, y+dy, color, matrix)
        ...
        for y in xrange(DIGIT_DATUM_HEIGHT):
            if datum.getPixel(x, y) > 0:
                if y >= DIGIT_DATUM_HEIGHT / 2:
                    cnt1 += 1
    if cnt1 > 0.2 * (DIGIT_DATUM_HEIGHT*DIGIT_DATUM_WIDTH/2):
        return  1
    return 0
def check_top(datum):
        ...
        for y in xrange(DIGIT_DATUM_HEIGHT):
            if datum.getPixel(x, y) > 0:
                if y >= DIGIT_DATUM_HEIGHT / 2:
                    ...
                    cnt2 += 1
    return cnt1 > cnt2 and 1 or 0


def check_side(datum):
...
def enhancedPacmanFeatures(state, action):
    ...
    if state.getFood().count() != 0:
        features["food"] = 1.5/min([util.manhattanDistance(state.getPacmanPosition(), _) for _ in state.getFood().asList()])
    else:
        features["food"] = 10
    features["ghost"] = 1.5*min([util.manhattanDistance(state.getPacmanPosition(), _) for _ in state.getGhostPositions()])
