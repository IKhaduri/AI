...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            ...
            self.beliefs[p] *= emission_model[true_distance]
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        res = util.Counter()
        all_pos = ((self.getPositionDistribution(self.setGhostPosition(gameState, p)).items(),  p)
                   for p in self.legalPositions)
        for state, pos in all_pos:
            for cur_pos, prob in state:
                res[cur_pos] = res[cur_pos] + \
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = [self.legalPositions[i % length] for i in xrange(self.numParticles)]
    ...
    def observe(self, observation, gameState):
        ...
        pacman_position = gameState.getPacmanPosition()
        if noisy_distance is None:
            self.particles = [self.getJailPosition() for _ in xrange(self.numParticles)]
            return
        all_weights = util.Counter()
        for particle in self.particles:
            all_weights[particle] += emission_model[util.manhattanDistance(particle, pacman_position)]
        all_weights.normalize()
        if all_weights.totalCount() > 0:
            self.particles = [util.sample(all_weights) for _ in xrange(self.numParticles)]
            return
        self.initializeUniformly(gameState)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.particles = list([particle for particle in itertools.product(*\
                        [self.legalPositions for _ in range(self.numGhosts)])] *\
                         (self.numParticles/self.numGhosts))[:self.numParticles]
    ...
    def observeState(self, gameState):
        ...
        all_weights = util.Counter()
        for particle in self.particles:
            weight_mult = [emission_models[i][util.manhattanDistance(particle[i], pacman_position)]
                           for i in xrange(self.numGhosts) if noisy_distances[i] is not None]
            tmp = 1
            for w in weight_mult:
                tmp *= w
            all_weights[particle] += tmp
            if tmp > 0:
                flag = 1
        all_weights.normalize()
        if flag == 1:
